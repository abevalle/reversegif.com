...
export async function fetchBlogPosts(page = 1, pageSize = 10) {
  try {
    ...
    console.log('API response for fetchBlogPosts:', result);
...
export async function fetchBlogPostBySlug(idOrSlug) {
  try {
    ...
    console.log('API response for fetchBlogPostBySlug:', JSON.stringify(data, null, 2));
    ...
    if (isNaN(idOrSlug)) {
      ...
      if (!data.data) {
        console.error('Invalid API response structure for ID query:', data);
        return null;
      }
      return data.data;
    }
...
export async function fetchFeaturedBlogPosts(limit = 3) {
  try {
    ...
    const data = await response.json();
    console.log('API response for fetchFeaturedBlogPosts:', data);
